...
```{r}
Regression <- lmer(data = d7, CasePerTest ~ Fitted2 + stringency_index + Under_30 + PercentageForeignersIkke + PercentageForeigners_vest + Retail_Change_lag + Transit_Change_lag + Grocery_Change_lag + Befolkningstæthed + Residential_Change_lag + (1 | Kommune ) + (1 | CountDate), REML = F, control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE), na.action = na.exclude)
Summary(Regression)
```
...
```{r}
RSS_R <- sum(Regression$residuals^2)
